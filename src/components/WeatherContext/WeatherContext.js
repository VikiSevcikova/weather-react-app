...
export const WeatherProvider = (props) => {
  ...
  const fetchWeather = async () => {
    const daily = `https://api.openweathermap.org/data/2.5/weather?q=${state.city
      .replace(" ", "+")
      .toLowerCase()}&appid=${API_KEY}&units=${
      state.isMetricUnit ? "metric" : "imperial"
    }`;
    const forecast = `https://api.openweathermap.org/data/2.5/forecast?q=${state.city
      .replace(" ", "+")
      .toLowerCase()}&appid=${API_KEY}&units=${
      state.isMetricUnit ? "metric" : "imperial"
    }`;
  ...
  return (
    <WeatherContext.Provider
      value={{ dailyWeather, forecastWeather, state, dispatch }}
    >
      {props.children}
    </WeatherContext.Provider>
  );
